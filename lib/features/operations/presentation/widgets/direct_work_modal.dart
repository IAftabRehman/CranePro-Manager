...
class _DirectWorkModalState extends ConsumerState<DirectWorkModal> {
  ...
  final _workCommissionController = TextEditingController(); // Partner commission
  ...
  void initState() {
    ...
    if (widget.initialData != null) {
      ...
      _workCommissionController.text = data.workCommission > 0 ? data.workCommission.toString() : '';

    }
    // Logic for real-time commission calculation
    _earningsController.addListener(_calculateCommission);
    _workCommissionController.addListener(_calculateCommission);
  }

  void _calculateCommission() {
    final total = double.tryParse(_earningsController.text) ?? 0.0;
    final paid = double.tryParse(_workCommissionController.text) ?? 0.0;
    setState(() {
  ...
  void dispose() {
    ...
    _workCommissionController.dispose();
    super.dispose();
  }
  ...
  Future<void> _handleSave() async {
    ...
    try {
      // No Firebase Auth — use a generic operator identity
      ...
      final comm = double.tryParse(_workCommissionController.text) ?? 0.0;
      final net = (gross - comm).clamp(0, double.infinity).toDouble();
      ...
      final workOrder = WorkOrderModel(
        ...
        operatorName: widget.initialData?.operatorName ?? operatorName,
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const DirectInputLabel(text: 'Commission (AED)'),
                      CraneInput(
                        controller: _workCommissionController,
                        hintText: '0',
                        keyboardType: TextInputType.number,
                        prefixIcon: const Padding(
                          padding: EdgeInsets.symmetric(vertical: 14, horizontal: 8),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  const Text('Net Earnings:', style: TextStyle(color: Colors.greenAccent, fontWeight: FontWeight.bold, fontSize: 12)),
